...
const OrderList: React.FC = () => {
  ...
  const SortHeader: React.FC<{ label: string; sortKey: SortKey; className?: string }> = ({ label, sortKey, className = "" }) => {
    ...
    return (
      <th 
        className={`p-4 font-medium cursor-pointer hover:bg-gray-100 transition-colors select-none group whitespace-nowrap ${className} ${isActive ? 'bg-blue-50/50' : ''}`}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 text-gray-500 text-xs border-b border-gray-200">
              ...
              <th className="p-4 font-medium whitespace-nowrap">确认号</th>
              <th className="p-4 font-medium whitespace-nowrap">车辆信息</th>
              <SortHeader label="取车信息" sortKey="pickupTime" />
              <SortHeader label="还车信息" sortKey="returnTime" />
              
              <th className="p-4 font-medium whitespace-nowrap">租车用户</th>
              <th className="p-4 font-medium whitespace-nowrap">订单金额</th>
              <th className="p-4 font-medium whitespace-nowrap">订单备注</th>
              
              <SortHeader label="更新时间" sortKey="updateTime" />
              
              <th className="p-4 font-medium whitespace-nowrap">订单状态</th>
              <th className="p-4 font-medium text-right whitespace-nowrap">操作</th>
            </tr>
          </thead>
...
const OrderRow: React.FC<{ order: Order }> = ({ order }) => {
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-blue-50/30 transition-colors">
      ...
      {/* Remarks */}
      <td className="p-4 align-top text-gray-500 text-xs max-w-[150px] truncate">
        {order.remarks || '--'}
      </td>
